    ...
    def forward(
        ...
        if not self.is_decoder:
            ...
            attention_mask = attention_mask.view(
                attention_mask.size(0) * self.config.n_context, -1
            )
        ...
        if not self.is_decoder:
            if not return_dict:
                ...
                last_hidden_state = last_hidden_states.view(
                    self.config.bsz, -1, last_hidden_states.size(-1)
                )
                output = tuple(
                    last_hidden_state,
                    *output[1:],
                ...
                output.last_hidden_state = last_hidden_state.view(
                    self.config.bsz, -1, last_hidden_state.size(-1)
                )
...
class FiD(T5ForConditionalGeneration):
    ...
    def get_crossattention_scores(
        self, n_passages, mask, labels, ids, mode="all", mask_query=None
    ):
        """
        ...
        if "probs" in mode or "all" in mode:
            self.aggregate_value(
                probs, mask, labels, n_passages, ids, mask_query, output, prefix="probs"
            )
        if "norms" in mode or "all" in mode:
            self.aggregate_value(
                norms, mask, labels, n_passages, ids, mask_query, output, prefix="norms"
            )
        return output
    ...
    def aggregate_value(
        ...
        ntokens_wquery = (
            mask.sum(dim=[2]) * n_layers * (~(labels == -100)).sum(dim=[1])[:, None]
        )
        ...
        scores_wquery_sepmask = scores.masked_fill(~(ids == 1)[:, None], 0).sum(
            dim=[1, 3]
        )
        ...
        if mask_query is not None:
            output[f"{prefix}woquery"] = self.get_woquery_score(
                scores, mask_query, mask, labels, n_layers
            )
