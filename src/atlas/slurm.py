...
def init_distributed_mode(params):
    ...
    params.is_slurm_job = (
        "SLURM_JOB_ID" in os.environ and "WORLD_SIZE" not in os.environ
    )
    ...
    if params.is_distributed:
        ...
        p2 = subprocess.Popen(
            ["grep", "default"], stdin=p1.stdout, stdout=subprocess.PIPE
        )
        p1.stdout.close()
        gloo_socket_ifname = (
            subprocess.check_output(["awk", "{print $5}"], stdin=p2.stdout)
            .decode("utf-8")
            .strip()
        )
        p2.stdout.close()
