...
def load_retriever(opt, opt_checkpoint=None):
    ...
    if opt_checkpoint is not None:
        retriever_is_untied = (
            opt_checkpoint.query_side_retriever_training
            or opt.query_side_retriever_training
        )
    else:
...
def _convert_state_dict_from_dual_encoder_retriever(state_dict):
    ...
    for k, tensor in state_dict.items():
        if k.startswith("retriever"):
            new_state_dict[
                k.replace("retriever.contriever", "retriever.passage_contriever")
            ] = tensor
            new_state_dict[
                k.replace("retriever.contriever", "retriever.query_contriever")
            ] = tensor
        else:
...
def load_reader(opt):
    ...
    if not opt.retrieve_only:
        ...
        if (
            opt.compute_crossattention_stats
            or "eval" in opt.gold_score_mode
            or "std" in opt.gold_score_mode
        ):
...
def _load_atlas_model_state(opt, opt_checkpoint, model, model_dict):
    model_dict = {
        k.replace("retriever.module", "retriever").replace("reader.module", "reader"): v
        for k, v in model_dict.items()
    }
    if (
        opt.query_side_retriever_training
        and not opt_checkpoint.query_side_retriever_training
    ):
    ...
    if opt.use_file_passages:  # dont load retriever if in use_file_passages mode
        model_dict = {
            k: v for k, v in model_dict.items() if not k.startswith("retriever")
        }
...
def load_atlas_model(dir_path, opt, reset_params=False, eval_only=False):
    ...
    if not reset_params:
        optimizer, scheduler, retr_optimizer, retr_scheduler = set_optim(
