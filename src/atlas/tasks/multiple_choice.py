...
class Task(BaseTask):
    ...
    def __init__(
        self, opt: Options, tokenizer: PreTrainedTokenizerBase, *args, **kwargs
    ):
        super().__init__()
    ...
    @staticmethod
    def get_multiple_choice_question_prompt(
        tokenizer, question, choices, maximum_length=356
    ):
        ...
        choices_wseparator = " ".join(
            [f"({L}) {T}" for L, T in choices.items()]
        ).strip()
        question_with_options = _get_prompt(question, choices_wseparator)

        if _length_in_tokens(question_with_options) > maximum_length:
            max_qlen = maximum_length - _length_in_tokens(
                _get_prompt("", choices_wseparator)
            )
    ...
    def get_permutations(example, permutations_type):
        ...
        for ordering in orderings:
            ...
            permuted_answer = [
                k for k, ans in permuted_options.items() if ans == options[answer]
            ][0]
