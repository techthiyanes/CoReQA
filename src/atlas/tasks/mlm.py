...
class Task(BaseTask):
    ...
    def apply_mlm_noise(
        ...
        tokens = tokenizer(
            text, add_special_tokens=False, max_length=max_input_length, truncation=True
        )["input_ids"]
        length = len(tokens)
        ...
        for i, (inp_length, out_length) in enumerate(
            zip(nonnoise_span_lengths, noise_span_lengths)
        ):
