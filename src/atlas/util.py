...
def cast_to_precision(model, precision):
    if precision == "fp32":
        ...
        raise ValueError(
            f"unsupported precision {precision}, must be one of fp32, fp16, bf16"
        )
    return model
...
class IndexRefreshScheduler(object):
    def __init__(
        self, format_str: str, freeze_retriever_steps: int, train_retriever: bool
    ):
        ...
        self.steps2rates = IndexRefreshScheduler.parse_index_refresh_schedule_string(
            format_str
        )
    ...
    def is_time_to_refresh(self, step):
        if not (
            self.train_retriever or step == 0
...
def set_optim(opt, model):
    ...
    if opt.train_retriever:
        retr_optimizer = optim_class(
            params=model.retriever.parameters(), lr=opt.lr_retriever, **optim_args
        )
...
def compute_grad_stats(model):
    with torch.no_grad():
        ...
        res = {}
        res["skip_example"] = (
            torch.any(torch.isinf(stats)) or torch.any(torch.isnan(stats))
        ).item()
